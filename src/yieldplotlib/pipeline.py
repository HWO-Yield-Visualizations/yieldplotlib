...
def ypl_pipeline(runs):
    ...
    ypl.compare(axes["A"], runs, "angdiam",
                plot_type="hist", **hist_kwargs)

    axes["B"].set_ylabel("# Targets")
    ypl.compare(axes["B"], runs, "MV",
                plot_type="hist", **hist_kwargs)

    axes["C"].set_ylabel("# Targets")
    ypl.compare(axes["C"], runs, "exp_time_det",
                plot_type="hist", **hist_kwargs)

    ypl.compare(axes["D"], runs, "star_dist", y="star_L",
                plot_type="scatter")

    axes["E"].set_ylabel("# Targets")
    ypl.compare(axes["E"], runs, "exp_time_char",
                plot_type="hist", **hist_kwargs)


    ypl.compare(axes["G"], runs, "star_dist",
    ...
    run_labels = ["EXOSIMS" if isinstance(run, EXOSIMSDirectory) else "AYO"
                  for run in runs]
    plot_hist(
            temps, planet_bins, runs, run_labels, ax=axes["H"])
